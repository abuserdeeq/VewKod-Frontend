...
import { augmentWithAst } from "./astAugment.js";
...
export function generateLocalExplanation(code, language) {
  ...
  let structure = analyzer.analyzeStructure(lines, symbolTable);
  let issues = analyzer.findIssues(lines, symbolTable);

  // If this language's tree-sitter grammar has finished loading
  // (see src/localEngine/core/treeSitterEngine.js — kicked off in
  // the background from App.jsx on startup), cross-check/extend the
  // regex analyzer's output with AST-verified functions/classes and
  // issues (unreachable code, empty catch/except). Stays synchronous
  // and falls back to the regex-only result above whenever the
  // grammar isn't warm yet, isn't available for this language, or
  // parsing fails for any reason — see astAugment.js.
  ({ structure, issues } = augmentWithAst(detectedLanguage, code, structure, issues));
  ...
  explanation += `\n---\n\n`;
  explanation += `*Generated by VewKod Local Explanation Engine V4 (language-specific analyzers, cross-checked against a real syntax tree via tree-sitter where available). This fallback runs locally when the AI backend is unavailable.*`;
